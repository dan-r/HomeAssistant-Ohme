...
from custom_components.ohme.const import DOMAIN, DATA_CLIENT, DATA_COORDINATORS, COORDINATOR_CHARGESESSIONS, COORDINATOR_ACCOUNTINFO
...
def mock_client():
    ...
    client.is_capable.side_effect = lambda x: x in ["buttonsLockable", "pluginsRequireApprovalMode", "stealth"]
    return client
...
async def test_ohme_configuration_switch(mock_hass, mock_client, mock_coordinator):
    switch = OhmeConfigurationSwitch(mock_coordinator, mock_hass, mock_client, "lock_buttons", "lock", "buttonsLocked")
    await switch.async_turn_on()
    mock_client.async_set_configuration_value.assert_called_once_with({"buttonsLocked": True})
    mock_client.async_set_configuration_value.reset_mock()
    await switch.async_turn_off()
    mock_client.async_set_configuration_value.assert_called_once_with({"buttonsLocked": False})
...
async def test_ohme_solar_boost_switch(mock_hass, mock_client, mock_coordinator):
    ...
    await switch.async_turn_on()
    mock_client.async_set_configuration_value.assert_called_once_with({"solarMode": "ZERO_EXPORT"})
    mock_client.async_set_configuration_value.reset_mock()
    await switch.async_turn_off()
    mock_client.async_set_configuration_value.assert_called_once_with({"solarMode": "IGNORE"})
