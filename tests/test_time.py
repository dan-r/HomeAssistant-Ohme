...
from custom_components.ohme.const import DOMAIN, DATA_CLIENT, DATA_COORDINATORS, COORDINATOR_CHARGESESSIONS, COORDINATOR_SCHEDULES
    ...
    hass.data = {
        DOMAIN: {
            'test@example.com': {
                ...
                DATA_CLIENT: MagicMock(async_apply_session_rule=AsyncMock(), async_update_schedule=AsyncMock())
...
def target_time_entity(mock_hass):
    coordinator = mock_hass.data[DOMAIN]['test@example.com'][DATA_COORDINATORS][COORDINATOR_CHARGESESSIONS]
    coordinator_schedules = mock_hass.data[DOMAIN]['test@example.com'][DATA_COORDINATORS][COORDINATOR_SCHEDULES]
...
async def test_async_added_to_hass(target_time_entity):
    with patch.object(target_time_entity.coordinator_schedules, 'async_add_listener', return_value=AsyncMock()) as mock_add_listener:
